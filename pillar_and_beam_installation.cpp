...
{
  // 왼쪽 끝 부분이 기둥 위에 있는 경우
  if (pillarss[y - 1][x] == 1)
    return true;
  // 오른쪽 끝 부분이 기둥 위에 있는 경우
  if (pillarss[y - 1][x + 1] == 1)
    return true;
  // 양쪽 끝 부분이 다른 보와 동시에 연결 된 경우
...
vector<vector<int>> solution(int n, vector<vector<int>> build_frames) {
    vector<vector<int>> beamss(n + 1, vector<int> (n + 1, 0));
    vector<vector<int>> pillarss(n, vector<int> (n + 1, 0));
